 ...
 * Whoami Cfg tab. 1.0 = baseline (`095e41a`); 1.1 = EPD grey + wifi CJK fixes; 1.2 = review round (whoami cfg-epoch P1, OTA NTP, sleep/OTA mutex); 1.3 = wifi scan vs reconnect-loop fix (-2); 1.4 = 4_2 async scan (no UI blocking) + scan-pick never clobbers a slot; 1.5 = bounded wifi auto-reconnect (5 tries + backoff, then idle); 1.6 = idle sleep 10 min while USB/charging (VBUS in), else 5.
 */
#pragma once
