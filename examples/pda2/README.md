...
- **测试机 #2**（2026-09-10 入池，COM6）：**V1.1 音频选配版**——有 DRV2605（V1.1 批次特征）、**无 A7682E**（AT 无响应 + 原厂固件菜单门控把 A7682E 项替换为 PCM5012 app）、**PCM5102A 实测出声**（耳机播放 `/iphone_call.mp3`）。触摸为 CST328 @0x1a（机 #1 是 Hynitron CST66xx/hyn 驱动；两者同地址不同驱动，LilyGo issue #37）。**音频能力**：输出可用（仅耳机，GPIO41 需拉高）；PDM 麦克风**在板但低灵敏度约 20~30dB——经 200Hz 高通+增益链后 STT 实测通过**（GLM-ASR-2512 逐字转写跟读句，§16.1），语音输入可立项；无扬声器。TTS 播放硬件路径在本机可用，见 issue_list §16
