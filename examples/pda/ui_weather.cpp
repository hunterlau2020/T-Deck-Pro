...
static void setup_table(lv_obj_t *table, int tw)
{
    ...
    lv_table_set_col_width(table, 0, tw * 14 / 100);  // Time/Day
    lv_table_set_col_width(table, 1, tw * 24 / 100);  // Weather
    lv_table_set_col_width(table, 2, tw * 18 / 100);  // Temp
    lv_table_set_col_width(table, 3, tw * 14 / 100);  // Humidity
    lv_table_set_col_width(table, 4, tw * 16 / 100);  // Wind
    lv_table_set_col_width(table, 5, tw * 14 / 100);  // Rain
...
static void update_forecast_tables()
{
    ...
    if (hourly_table && hourly_count > 0) {
        ...
        lv_table_set_row_cnt(hourly_table, visible + 1);
    ...
    if (daily_table && daily_count > 0) {
        ...
        lv_table_set_row_cnt(daily_table, visible + 1);
        for (int i = 0; i < visible; i++) {
            int r = i + 1;
            int si = start + i;
...
static void update_ui()
{
    ...
    if (weather_icon)
        lv_img_set_src(weather_icon, weather_icon_img(cur.icon));
...
void ui_weather_enter(lv_obj_t *parent)
{
    ...
    weather_icon = lv_img_create(temp_row);
    lv_img_set_src(weather_icon, &img_w_cloud);
