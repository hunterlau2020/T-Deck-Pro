...
static void password_ta_event_cb(lv_event_t *e)
{
    ...
    if (copyToBuffer) {
        ...
        if (strlen(password) > 0) {
            strncpy(wifi_password, password, strlen(password));
            printf("PWD:%s\n", wifi_password);
        } else {
            printf("PWD IS EMPTY!\n");
        }
...
static lv_obj_t *password_text_crate(lv_obj_t *parent)
{
    ...
    strncpy(wifi_password, WIFI_PASSWORD, strlen(WIFI_PASSWORD));
#endif
...
static void wifi_connect_event(lv_event_t *e)
{
    ...
    } else if ( strlen(wifi_password) < 8) {
...
#endif
#ifdef WIFI_SSID2
        if (strcmp(wifi_ssid, WIFI_SSID2) == 0) {
            strncpy(wifi_password, WIFI_PASSWORD2, strlen(WIFI_PASSWORD2));
            printf("Copy ssid:%s password to buffer:%s\n", wifi_ssid, wifi_password);
        }
#endif
    }
