...
static void _msg_ta_cb(lv_event_t *e)
{
    ...
    } else if (lv_indev_get_type(indev) == LV_INDEV_TYPE_KEYPAD) {
        return ;
    }
    ...
    if (code == LV_EVENT_KEY) {
        ...
        if (key == LV_KEY_ENTER) {
            lv_textarea_del_char(input_textarea);
    ...
    if (code == LV_EVENT_CLICKED && lv_indev_get_type(indev) == LV_INDEV_TYPE_POINTER) {
        ...
        lv_obj_align(quit_btn, quit_btn_align, quit_btn_x_ofs, quit_btn_y_ofs);
#endif
    } else  if (code == LV_EVENT_CLICKED && lv_indev_get_type(indev) == LV_INDEV_TYPE_ENCODER) {
        if (edited) {
