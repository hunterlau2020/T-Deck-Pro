...
- **现象**（真机报告）：首次进入 Level APP 后第一次按 Enter 总是没反应，
  第二次才开测；再次进入则正常。
- **根因**：`entry_lt()` 进屏即发起 history 拉取（进屏后首张屏幕的
  第一个 HTTP(S) 往返最慢，`s_lt_task` 可挂数秒），键盘轮询里
  `if (s_lt_task) continue;` 把该窗口内到达的 Enter **静默吃掉且无
  任何反馈**。再次进入时链路已热、窗口毫秒级，碰不到。
- **修复**：Enter 在请求在飞时不再丢弃，置 `s_enter_pending` 缓冲，
  任务一空（lt_consume 后的轮询检查）立即补发开测；quiz 的 1-4 键
  在飞时仍有意丢弃（屏上题目已作答，缓冲会答错题）。lt_start 前置
  拒绝（WiFi/配置缺失）补串口打点，下次不再盲诊。
- 版本 v1.17 → **v1.18**；COM5 6/6 VERIFIED，串口确认
  `[FW] v1.18` + `self-attest: mark valid ok`。
...
- **背景**：v1.11 后用户报告"还是老样子"（双光标 + not connect）。
  复核 `ui_scr_mrg.c`：push 时旧屏 exit 确实执行（v1.11 修复在路径
  上），create 每次 push 重跑——推断残留现象另有层：
- **"一直 not connect"的显示层根因**：状态行只在进屏时写一次，
  之后管理器把链路重连回来（GOT_IP）**没有任何事件更新屏幕**——
  用户看到的 "Not connected" 可能是陈旧快照。**修复**：
  `wifi_cfg_keyboard_poll` 加链路状态跟踪（仅真实变化时更新，
  不覆盖 Connecting/Scan 等进行中横幅），串口同步打
  `[WiFi] link state -> ...` 便于核对。
- **双光标二分法**：v1.11 已在 `draw_cursor()` 的 `cursor.show`
  绘制门掐死仍"看见闪烁"→ **全灭 4_1 光标**（两个 ta 的 show=0 +
  CURSOR part TRANSP，set_field 不再发 FOCUSED——它唯一残余效果
  就是 start_cursor_blink），焦点指示只靠 ">" 标签。若用户仍见
  "闪烁"，即可断定闪烁源不是 textarea 光标（另行排查显示层）。
- 版本 v1.11 → **v1.12**。教训：**修复"没生效"时先验证修复代码
  真的会被执行（读 scr_mgr 源码而非注释），再怀疑现象的观测层
  （显示的是不是陈旧状态）。**
