...
- **测试机 #2 麦克风终判：不可用**（issue_list §16.1，`test_pdm_mic`
  v1..v11 真机探针）：GPIO41 高/低/浮空 × PDM 左/右半位流 × 受控近讲，
  ~200 个窗口语音从未超过环境底噪（逐秒 VU），电气底噪恒在 → 麦克风
  未焊或未接声孔，固件不可修；v1 时代"人声巨峰"经查过零率仅 40/s，
  系手持耦合非声学拾音。本机音频定格：**输出可用（耳机）、输入不可用、
  无扬声器**。探针固化了 4 条音频开发铁律（GPIO41=输出段电源、Audio
  驱动重建须复刻构造配置、PDM 只能借 I2S0、回放统一 44.1k 立体声），
  直接服务后续读信 TTS。同日：整片 16MB Flash 备份（32/32 块零重试，
  md5 `45cfb17d...`）+ `script/flash_backup.py` 入库；机 #2 出厂 demo
  已被探针覆盖（其余分区在备份内）。
