...
LilyGo **T-Deck-Pro V1.1** firmware repository. ESP32-S3, 16MB Flash + 8MB PSRAM, GDEQ031T10 3.1" E-Paper (240×320 monochrome), TCA8418 4×10 keypad, CST226SE touch. Targets are independent example sketches under `examples/`; `pda2` is the active PDA application (built on top of `factory`). The consolidated `examples/allinone` was **cancelled by user decision (2026-08-19)** — pda2 is the final integrated firmware (3-page 19-entry menu since 2026-08-22: PenPal letter app alone on page 3); `docs/allinone-design.md` is archived as pda2-evolution reference. **Hardware gotchas (issue_list §3)**: the 4G (A7682E) board variant has NO PCM5102A DAC (MP3 playback impossible, verified by `test_i2s_probe`); SD cards must be FAT16/FAT32 (exFAT shows 0MB, Setting screen now hints this).
...
- **Latest review rounds** (rounds 29-31 all Codex **A 全量接受**; details in each result file): round 29 `d22007d..4c3c9b1` (native dropdown provider + per-provider keys + OPENROUTER_KEY rename + custom clears), round 30 `a2ecd7b` (key.custom dead store + KEEP-IN-SYNC note), round 31 `764e7bf..980b6df` (CST-8 local month boundary + scan overlay hidden on screen cover + test_keypad mirror note). **Results 2026-08-22** (Codex + Kimi double review, all **A** except one): third batch `6d26699..1473ef9` A (result filed under pre-rewrite range `3f654a5..4c3a331`; the request file had never been created — backfilled in `eefb2fd` and cross-linked), `de78338` A, `c27cb39..3475c9b` A (GPT follow-up 3 P2s closed, issue_list §9), `a924c4e`: Codex **C** — its P2 (SD hint asserting FAT32 for every mount failure) fixed in `c8f62f3` (issue_list §3.4 follow-up); Kimi A but argued the original copy was fine (divergence logged in the `c8f62f3` request; `a924c4e` is a pre-rewrite id, current HEAD = `23030c9`). Low backlog registered, not blocking: issue_list §7.4 (Trust toggle affects ALL HTTPS consumers, UI copy doesn't say so), §9.4 (cold-start + forecast-only shows no partial hint) — **both closed 2026-08-22** in the Low wrap-up batch (`a58a73c` status-line scope copy, `71fa528` forecast-only data_valid; `c8f62f3` got Codex **A**, `71fa528..a58a73c` got **C** — its P2, zero-slot forecast still marked valid, closed by `141942d`). **Pending requests**: `141942d` (weather zero-slot fix) + `acc3893` (PenPal fix-round). **PenPal**: implementation `b48f584..5329383` (API client with shared-TLS `http_apply_tls`/`http_ensure_time` additive exports → R9 residual-thread query `16c13e3`, v3.2 null rows SHOWN + THREAD read-only via single-param `thread_root_id` → screen UI `ui_penpal`×3, single-queue async framework, first cancellable waitbox, SEND idempotency + in-flight COMPOSE edit lock → menu third page `5329383` with `menu_page_apply()` generalized pagination + `scripts/gen_img_penpal.py` icon) got Codex **C** 2026-08-22: 2×P1 (memset UB on `std::string`-holding structs; create-time auto-sync invalidated by entry gen++ — **scr_mgr calls create() at REGISTER time, `ui_scr_mrg.c:33`** — with busy never released) + P2 (READ-Close zombie worker pile-up) → all fixed same-day in `acc3893` (value-init rule "no memset on any `pp_*_t`"; auto-sync moved into entry() behind `s_pp_autosynced`, reset on destroy AND cfg-save; atomic `s_pp_inflight` cap 2 counted before xTaskCreate; stale-drop releases busy when it owns `s_pp_busy_gen` — closes the sibling background-SEND-exit leak; issue_list §10). Device regression = design §7 checklist + 4 fix-path items (auto-sync busy release / background-send exit not busy-stuck / Close-retry cap message / Cfg-save→re-entry resync), all ⏸ 待用户实测. Earlier frontier (rounds 22-28) covered: dual-slot NVS atomic save, WiFi scan critical-section + busy generation, Sleep countdown gated by the screen's OWN EPD flush sequence, AI Chat dynamic bodies + SPIFFS atomic log + turn-paired 8KB multi-turn context + New confirmation, usage stats as a mutex-guarded single NVS blob (V3 monthly reset), AI Config save-failure msgbox + Test billing transparency, secrets chain + API-key compensating controls. Read those commit messages before extending the corresponding area. Earlier frontier (rounds 22-28) covered: dual-slot NVS atomic save, WiFi scan critical-section + busy generation, Sleep countdown gated by the screen's OWN EPD flush sequence, AI Chat dynamic bodies + SPIFFS atomic log + turn-paired 8KB multi-turn context + New confirmation, usage stats as a mutex-guarded single NVS blob (V3 monthly reset), AI Config save-failure msgbox + Test billing transparency, secrets chain + API-key compensating controls. Read those commit messages before extending the corresponding area.
