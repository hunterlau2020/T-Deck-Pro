...
| 2 | 首次保存（无旧配置） | slot 0 完整写入，active=0（putUChar 写 0 返回 1，与失败返回值 0 可区分） | `Saved`；重启后 load 读 slot 0 |
...
| 9 | 旧固件平键迁移 | 旧固件写过 `base/model/key`（无 active 键）：load 回退读平键；save 后写槽 + active，平键不再使用 | — |
| 10 | 保存空 Base | 允许（写空串是合法 NVS 值）；load 时 b.length()==0 → 回退默认值路径 | — |
