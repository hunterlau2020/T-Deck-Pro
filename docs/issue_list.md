...
### 16.1 麦克风判定：不可用（2026-09-10 晚，`test_pdm_mic` 探针 v1..v11）

- **方法**：无头探针循环（嘟→录音 6s→逐秒 VU→滴滴判定→必回放 44.1k
  立体声），11 版迭代覆盖：GPIO41 高/低/浮空三态 × PDM 左/右半位流 ×
  直流去除/增益归一回放 × 受控近讲（对嘴 5~10cm）。
- **结果**：~200 个窗口，语音**从未超过环境底噪**（所有秒 143~1065）；
  电气底噪恒在（直流基座 -1494 不变）→ PDM 前端活着，声学通路不存在。
- **v1"人声巨峰"证伪**：peak 20000~34000 但过零率仅 40/s（真语音应为
  数百/s），与手持设备时机械/电气耦合特征一致，非声学拾音。
- **结论**：麦克风元件未焊或未接声孔，固件不可修。本机音频能力最终
  定格：**输出可用（耳机，PCM5102A 实测）、输入不可用、无扬声器**。
