...
**闭环（2026-09-18）**：拆段已执行（三份申请，`5d1dbfb`）；命名口径
收拢进 `docs/reviews/README.md` 目录约定（非连续提交集用描述性文件名 +
文件内列明 commit，连续区间沿用区间记法）。

---

## 28. Level APP 中文选项渲染：字库覆盖必须按全量语料而非抽样（2026-09-18，v1.16）

**现象**：v1.15 真机验收发现定级测词汇题选项（words 表 `meaning_zh`
首义）渲染为空白——`ui_leveltest` 沿用 ASCII mono 资产字体。

**关键判断**：第一反应是套 penpal 的 `lv_font_simsun_16_cjk` 热切换，
但**实拉一卷即缺 11 字**（围/谅/广/袋/鼠/浓/缩/习/惯/险/逆）——内置
simsun 只有约 1000 常用汉字，而题库是随机抽题，抽样验证会漏。正确
口径：**从 DB 全量提取字符并集**（21311 词 meaning_zh 全串 + 语法题库
源码字符 = 3955 字），lv_font_conv 生成 `Font_Hanzi_16`（SimSun 16px
bpp4，4057 字形含 ASCII，+488KB flash），**生成后回读 .c 文件核对
0 缺字**再入库。

**教训**：①动态服务器语料的 CJK 字库覆盖以**全量语料并集**为准，
抽样（拉一卷/几卷）只能证伪不能证真；②生成物必须回读验证（管线：
fontTools 从 ttc 抽 ttf → Node 进程内调 lv_font_conv CLI → 正则回读
`U+XXXX` 对账）；③`options[48]` 遇超长释义（106/21311 词首义 >48B，
最长 151B）由 `s_copy_disp` UTF-8 边界截断 + "..."，缓冲无需扩——
单行 30px 行高本就只能显示 ~13 汉字。

**遗留**：题库后续新增词汇超出 3955 字全集会缺字（TODO 有重生成
指引）；IPA `ː` 与一个 PUA 私有区字符有意不含（SimSun 无字形）。

---

## 29. Level APP 整卷流提交 400：契约从固定题数改为终止事件（2026-09-18，v1.17）

**现象**：真机答到第 10 题被客户端强制提交，服务端 400。

**根因**：v1.13 按旧范例步骤 ⓯（整卷 10 题）实现；服务端评审（五十八）
后 staircase 契约收紧——`grade_staircase` 复算并要求 answers 恰为
**终止事件消费的前缀**（登顶/地板/二次降级/16 题上限任一），混合作答
10 答多半未终止 → 400 "session incomplete"；且终止后尾随答案也被拒
（"terminated early, but submitted N"）。新范例步骤 ⓰ 是 ESP32 参考
实现：单题 `next` 拉取 + 客户端本地镜像状态机 + 终止即提交。

**修复**：ui_leveltest 重写为单题循环（详见 CHANGELOG v1.17）。关键
纪律：**客户端状态机必须与服务端逐口径镜像**——先用脚本把设备侧
`lt_stair_step` 逐行转录成 Python，与 `grade_staircase` 对跑 20000 组
随机序列验证终止判定与消费数 0 偏差，再编译刷机；端到端 3 轮
（11/16/12 答）全 200。

**教训**：①外部契约变更时先重读参考实现再动客户端，别按记忆里的
旧契约修；②"提交条件"类协议语义（事件 vs 计数）是最容易踩的隐性
契约，字面读懂 400 文案（"no termination event reached"）即可定位；
③网络故障的中途策略是产品决策：设备选择"重试一次后放弃不提交"，
与演示脚本的模拟学生"盲答续推"有意不同（真人无法作答未拉到的题）。

---

## 30. Level APP 首次进入 Enter 无响应：在飞请求窗口静默吞键（2026-09-18，v1.18）

**现象**：首次进入 Level APP 第一次按 Enter 没反应，第二次才开测；再
进入正常。

**根因**：`entry_lt()` 进屏即发 history 拉取——开机后**第一个** HTTP(S)
往返最慢（建链/TLS/时间同步），`s_lt_task` 挂数秒；键盘轮询的
`if (s_lt_task) continue;` 在该窗口内到达的 Enter **静默丢弃、零反馈**。
再次进入链路已热、窗口毫秒级，故"只在首次复现"。

**修复**：Enter 意图缓冲（`s_enter_pending`），任务空闲即补发；1-4 作答
键在飞仍丢弃（屏上题已作答，缓冲会答错题——丢弃与缓冲的取舍按"意图
是否仍有效"判）。lt_start 前置拒绝路径补 `[LT] start refused` 串口打点。

**教训**：异步单飞 + 键盘轮询的组合里，`task-in-flight → continue` 是
隐形的吞键器；按键处理的每个丢弃分支都要问一句"用户此刻的意图下一帧
是否仍然成立"，成立就该缓冲而非丢弃。窗口型 bug（首现/复现差异）优先
找"冷热路径耗时差"，别急着怀疑事件分发。

---

## 31. ds4 三段评审处置：缓冲键跨页存活 + 详情 CJK 漏切 + 快径判负分支（2026-09-18，v1.21）

**评审**：三份分段申请（修复轮 delta / LevelTest 族 / newdict）全部 C
部分接受；核销面独立复核成立（阶梯状态机穷举 117586 前缀 0 偏差、
字库差集仅剩有意排除项、异步契约逐条符合）。

**发现与处置表**：

| 编号 | 级别 | 内容 | 处置 | 门 |
|---|---|---|---|---|
| L1 | P2 | `s_enter_pending` 只发不销，起步双 Enter → RESULT 同 tick 被补射覆盖成新 Q1 | 置位收紧 HOME 页 + `lt_render_question()` 进入即清 | G-合入（代码已修） |
| N1 | P2 | newdict 详情 examples/tail 漏切 CJK 字体，中文半句空白（v1.20 同款） | 改 `nd_set_text` 两行 | G-合入（代码已修） |
| F1′ | P3 | 终态快径误吞 `scanComplete()` 超时判负分支（驱动扫描在飞）早退跳 stop | 快径补 `esp_wifi_scan_stop()` + 注释收窄 | G-真机 |
| N2 | P3 | newdict 缓冲 Enter 跨页存活，详情返回后被补射 | `nd_render_detail`/`nd_tab_set` 清位 | G-真机 |
| N3 | P3 | entry 在飞时列表永不自动拉取 | **v1.20 结构性消除**（entry 不再自动拉取） | 闭环 |
| Nit-1 | — | 契约规则 7"超时自 delete"文本 vs 实现不符 | whoami/leveltest/newdict 三处补 `!= pdTRUE` 自释放 | 闭环 |
| Nit-2 | — | 设备侧无法输入中文却宣称中文子串搜索 | ui_newdict.h 能力注记 | 闭环 |
| Nit-2(lt) | — | `stem[128]` 截断与 exclude 错配隐患 | penpal_api.h 前提注记 | 闭环 |

**真机反例清单（§27 三条 + 本轮新增）**：④ 慢扫描环境（>6s）扫描中
Back ×10 再进 4_2 应立即出列表，无 `release deferred`/连续 `kick
failed`（F1′）；⑤ LT 起步连按两次 Enter 走完整场，结果页必须停留
（L1）；⑥ newdict 打开双语例句词条，中文半句必须出字（N1）。

**教训**：①"键缓冲"类状态必须绑定页面语义（能就地消费的页面才可置
位，页面切换即作废）——v1.18 修"吞键"时只想到不丢，没想到缓冲也会
**过期**；②同一文件里同类操作（四个详情标签）字体切换只做了两个——
机械模式应用要逐个清点，不能"顺手"；③框架 API 的返回值语义要按
**全部**返回路径读（scanComplete 有四条路，不止两条）。
