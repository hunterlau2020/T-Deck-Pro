...
- **验证**：2026-08-19 `examples/test_i2s_probe` 探针固件实测——SPIFFS 播放 1s mp3 全程解码正常（ID3→syncword→EOF 完整走完，串口日志可查），**耳机孔无任何声音**。ESP32-S3 无内置 DAC，板上无 DAC 芯片 = 无模拟音频输出路径，I2S 信号无处可去
