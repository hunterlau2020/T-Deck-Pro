...
│ ┌─────┐ ┌─────┐ ┌─────┐              │  笔友 icon 行（≤3 个，66×52）
│ │ S   │ │ M   │ │ t   │  S/Mei/terry │  首字母+名字平面文本（名字下划线，2026-08-26 去框）；点→COMPOSE(new)
│ └─────┘ └─────┘ └─────┘              │
│ status line（同步状态 / 错误）        │
│ ┌──────────────────────────────────┐ │
...
│ ├──────────────────────────────────┤ │  5 行 × 34px 超链式列表：无边框行
│ │ terry        08-20 14:33         │ │  + 每行一条 1px 分隔横线；点行→THREAD
│ │ usage test   sent                │ │
│ │ ...                              │ ││ └──────────────────────────────────┘ │
...
- **入页焦点重置**（2026-09-13）：`pp_cfg_prefill()` 首行重置 `s_cfg_focus` 到 Server URL（该变量 file-static、跨访问残留——修复“重进页面后 ⌫ 删错框”，`7ee150b`；经验沉淀见 issue_list §17）；
