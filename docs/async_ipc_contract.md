...
11. **多轮上下文轮次配对规则**（AI Chat 专属，评审要求 §1.7 文档化）：
    - 快照按**整轮**选择：一轮 = 一条 user + 紧随其后的 assistant（前一条**必须**是
      user 才配成一轮；否则该 assistant 为孤立消息，**停止窗口**——绝不让 assistant
      开头）
    - 尾部允许一条**孤立 confirmed user**（其回复未到达，如发送后掉电）
    - pending（未确认）气泡跳过；UI 标记（`(failed)`/`(truncated)`）剔除后再入上下文
    - 预算 8KB 按整轮字节计入，一轮放不下即停；最旧轮次先裁
    - 串口日志 `send: N context msgs`，恒为偶数（孤立 user 尾部除外）
