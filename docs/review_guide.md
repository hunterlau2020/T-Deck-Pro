...
2. **证据 > 票数**（承自参考 R17）：合入决定是**证据标签的函数，不是 reviewer
   ...
   **阻断力绑定证据严重级，不绑定 reviewer 身份**（v1.3，主审制裁定）：任一
   reviewer（含收口轮旁审）拿出带 `file:line` + 反例的**协议/架构 P0/P1** 均立即
   阻断；旁审的 P2/P3/实现合同/完备性意见仅建议，不单方面倒逼新设计版。主审
   （若设）= 收口轮 delta 协调人 + 实现合同归并人，**不是** P0/P1 的唯一闸门。
   实证：OTA v4 签名自指 P1 由三家命中而一家漏审——若彼时漏审者为主审且旁审
   无阻断权，该 P1 将随 A 流入实现（`…-3eb6f7e-qwen.md` §六盲点登记）。
...
11. **收口优先于完备**（设计稿专属，防完美主义，v1.3）：设计评审的目标是
    **G-开工**（协议可满足、按正文编码不会系统性变砖/误回滚），不是把实现期失败
    路径、RTOS 对象布局、编码大小写写成可粘贴固件规格——后者的门禁是 **L2 /
    G-合入**。无协议/架构 P0/P1 时必须给 L1+A（P2 进实现合同）；因"还可以更完
    整"给 C 再开设计版 = 违反本条。细则 §2.5。
...
| 对象 | 最低适用章节 |
|---|---|
| 设计稿评审（OTA / PenPal / WiFi 记忆槽 / AI provider 关联等 `docs/*.md`） | §0–§4、**§2.5（收口，强制）**、§6、§9（【D】条目）、§10；结论 = **L1 是否达标 + A/B/C**（含义见 §2.5.2，与代码 A/C 不同），无需虚构 CODE/HW 证据 |
...
| 级别 | 结论 | 最低条件 | 允许动作 |
|---|---|---|---|
| **L1 DOC-ALIGNED** | 设计稿内部 + 与 `CLAUDE.md`/`async_ipc_contract.md`/`issue_list.md` 一致 | **开工门槛，不是实现规格终稿**（§2.5）：承重协议可满足（签名可构造、回滚窗口不会系统性误杀健康镜像、TLS/密钥边界不自相矛盾）；字段/术语核到"JSON 键集与签名对象一致"；伪代码/JSON 不自指、不与 `issue_list.md` 冲突；确定性用语已标"目标"。**不要求** RTOS 对象布局、每个失败路径、编码大小写、校准矩阵穷尽——那些是实现合同，L2 核 | 允许据此开工编码；实现合同缺口列入设计稿 §10 或结果"实现合同"，绑实现 commit 的 G-合入 |
...
| **G-开工** | 开始写实现代码前（仅设计稿） | **L1 + 无未闭合的协议/架构 P0/P1**（§2.5）。残留实现合同 P2 不挡开工。属主可在模型给 C 但证据只支持实现合同时按 §0 原则 2 采纳 A |
...
### 2.3 代码审批三态与设计稿审批（v1.3 拆分）

**代码**评审结论 = **A 全量接受 / B 退回修订 / C 部分接受**（C 须注明保留/回退项 +
P0/P1 修复要求）。这是 **G-合入** 的输入。

**设计稿**同时出 L1 是否达标与 A/B/C，但 **A/C 的含义与代码不同**（§2.5.2）——
不得把代码那套"A 不得挂任何必修项"套到设计稿上，否则一条实现合同缺口就会逼出
新设计版 × 多方复审。**不新增第四种通用审批态**（v1.3 裁定：设计稿专用
"L1+A + 实现合同表"表达，代码侧 A/B/C 语义零改动）。
...
### 2.5 设计稿收口（防把 L1 当成实现规格终稿；v1.3 承重段）
...
**判定锚 = 最小修复**：须改承重规范才活 = P1；补一段与既有模块同构的调用/类型/
编码 = 合同。

**负面清单边界**（v1.3 收紧）：最小修复若触及**签名/回滚/密钥/TLS/变砖路径**
之一 → **强制归协议/架构类**（P0/P1），不得以"存在可行实现"为由归实现合同。
"是否存在可行的正确实现"判据仅用于非承重路径。

**不得把 `NO_CONTRACT`（没写布局）直接升 P1。** 未写 = 实现选型，L2 二选一
钉死。只有"正文已经唯一可推断的写法不可满足"（如签名对象含 `sig` 自身）才是
设计 P1。

**反假想级联（条件式，v1.3）**：本款不允许以"未来实现者可能写错"为前提作多跳
灾难推演（此类设计阶段封顶 P3）；**但若设计的缺失使任何实现都无法满足正文的
安全或协议声明，仍按第一行报告 P0/P1**——用"是否存在任何可行的正确实现"这个
可核实条件，取代"是否需要假设未来会写错"这个不可核实条件。

正例（挡 L1）：签名对象把 `sig` 编进待签输入（OTA v4 四方 P1）；自证窗口短于
现有 `setup()` 阻塞 → 健康镜像必被误回滚（OTA v3 P1）。
反例（不挡 L1）：未写主循环排空结果队列（PenPal 有同构先例）；未写结果结构是
POD 还是指针移交；sha256 大小写未钉死。

#### 2.5.2 设计稿的 A/B/C（与代码审批不同）

- **A + L1**：无协议/架构 P0/P1。允许附**实现合同清单**（P2/P3），绑实现
  commit 的 L2 / G-合入。那些项的门禁是代码合入，不是再改一版设计稿。
- **C**：仅当仍有未闭合的协议/架构 P0/P1，或承重方向仍错、必须改设计正文才能
  编码。C 的 required action 必须是"改哪一节规范"，不得是"请把实现细节再写
  完整"。
- **B**：推倒架构。

无协议/架构 P0/P1 时设计稿**必须给 L1+A**（可附实现合同）。因"还可以更完整"
给 C = 违反 §0 原则 11。代码评审维持原义：A 不得挂 P0/P1 必修；P2 应修进
`issue_list.md` 台账。

**实现合同跟踪表**（v1.3 收紧，防合同在开工后消失）——设计 A 的结果文件须含：

| 合同 ID | 正确性目标 | 绑定实现 commit/申请 | L2 验证证据 | G-合入状态 |
|---|---|---|---|---|
| IC-1 | 例：结果队列 drain 点挂 factory loop（PenPal 同构） | （实现申请锚） | （L2 对照结果） | （待/过） |

#### 2.5.3 复审范围 = delta（默认）

设计 vN 复审默认只核：`git diff <vN-1>..<vN>` 的规范性段落 + 作者列出的变化
点。未改章节当作已闭合，禁止当新架构重开。

扩面条件（须在结果头部声明段落与证据）：
① 修复引入回归（改措辞把已闭合声明改坏）；
② 新段与未改段直接矛盾；
③ **本轮拿出了独立的、带反例的协议/架构 P0/P1 证据且位于未改段**（v1.3 收紧
——"此前没检查"本身仍不是理由，证据才是）。
...
- **收口轮**（作者声明无架构变更、只闭合上一轮 P2/P3）：默认**一名** reviewer
  做 delta 闭合。但**任一 reviewer（含旁审）拿出带反例的协议/架构 P0/P1 时
  立即退出收口、回到多方轮**（v1.3 收紧——单 reviewer 是省力默认，不是屏蔽
  他人 P0/P1 的闸门，与 §0 原则 2 同源）。
- 多方（≥2 模型）触发：首轮；本轮有疑似协议/架构 P0-P1；或属主点名。
- **轮次兜底**（v1.3，裁定不设自动降级）：同主题拉扯满 **3 轮** → 触发**属主
  强制裁决**——裁决仍以"是否存在未闭合协议/架构 P0/P1"为条件（内容触发），
  **不得**因轮次计数自动给 A/开工（实证：OTA 签名自指恰在第 4 轮才被四方共同
  发现，硬顶 3 轮自动放行会漏掉它）。

#### 2.5.5 G-开工

见 §2.2 表。属主签字环节兜底（§0 原则 2）：模型给 C 但证据只支持实现合同缺口
时，属主可按 A 开工，并把该清单绑到实现申请。
...
已接受风险**（承自参考 R8）。上款适用于**代码**评审。设计稿见 §2.5：A 可附实现
合同 P2（门禁 = 实现 G-合入）；仅协议/架构 P0/P1 阻断 L1。**不得用本款强迫设计稿
对实现合同给 C**。

**§4.2.1 定级防膨胀铁律**（v1.3）：
① **静默表上调仅限运行态实体**（CODE/HW 证据），且仅适用于"设计正文已唯一确定
的运行态后果"——**不适用于**"靠假设未来实现错误"的假想级联（但确定性自相矛盾
如签名自指，无论载体是有声静默都是 P0/P1，不因 DOC 载体机械降级）；
② No Hypothetical Cascades 见 §2.5.1 条件式；
③ 多前提严格按 §0 原则 7b 折算可达性 3，禁主观升格。
...
设计稿 A 的入库轮还须把**实现合同表**（§2.5.2）逐条登记或移交实现申请（v1.3：
合同在设计 A 后消失 = 实现阶段的遗漏，等于把设计期过度阻断换了个方向）。
...
- **设计稿**：不把"可在实现时按既有先例补的合同缺口"报为挡开工项；报为"实现
  合同，绑 G-合入"（§2.5.1 第二行）。协议自指/按正文必误回滚/必变砖仍报 P0/P1。
- 设计收尾轮 P3/Nit 合计建议 ≤3；超出的进疑问清单，不占缺陷、不挡 L1。
- 对称三格①的"全区间"按对象裁剪：**设计稿复审默认 = 本版 diff + 作者变化点**
  （§2.5.3），非把未改章节当新架构；扩面须头部声明。代码评审仍为全区间。
...
【设计稿收口】【D+ALL】（v1.3，设计稿评审每轮必查）
□ 24. 无协议/架构 P0/P1 时是否给了 L1+A（P2 进实现合同）？因完备性给 C = 本条未过。
□ 25. 复审是否默认只核 diff/变化点？扩面是否声明了回归/矛盾段落或独立 P0/P1 证据？
□ 26. NO_CONTRACT / 与既有模块同构的缺口是否被错误升为挡 L1 的 P1？最小修复是否
      触及负面清单（签名/回滚/密钥/TLS/变砖）而被误归实现合同？
□ 27. 是否把伪代码当生产 C++ 做了 ABI/指针/布局静态分析、或把"实现者未来写错"
      的假想级联升格？（§2.5.1 条件式）
```
...
| 设计稿实现合同缺口 | 架构已达 L1，队列所有权/drain/编码钉等未写进正文 | 不挡 G-开工；L2 对照实现合同清单（§2.5.2 表） | 实现 commit 评审核清单；**禁止倒逼设计 vN+1** |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.3 | 2026-09-13 | **设计稿收口**（触发：OTA 设计 v1–v5 一日五轮，v5 起以实现合同缺口拒绝 L1、倒逼新版×多方；属主指出"追求完美"）。来源 = Grok 提案（骨架）+ Claude 提案（诊断/A-/合同命名/静默表方向）+ 三方裁定（qwen `…-6329f44-qwen.md` §五、Codex `…-codex.md` 合并规则、Claude `…-claude.md` 三拒绝）。**采纳**：§0 原则 11 收口优先；§0 原则 2 追加**严重级阻断纪律**（阻断力绑证据不绑身份——任一 reviewer 的协议/架构 P0/P1 均阻断；旁审 P2/P3 仅建议）；§2.5 三类发现+判定锚"最小修复"+**负面清单**（触及签名/回滚/密钥/TLS/变砖→强制协议类）+条件式反假想级联；§2.5.2 设计 A/B/C 拆分+**实现合同跟踪表**；§2.5.3 delta 复审+**第三扩面通道**（独立 P0/P1 证据）；§2.5.4 收口轮单 reviewer（**不屏蔽他人 P0/P1**）+轮次≥3 触发**属主裁决**（内容触发非自动放行）；G-开工门禁；§4.2.1 定级防膨胀铁律（静默表仅运行态+非假想级联）；§9 增【设计稿收口】24–27。**未采纳**（三方一致拒绝，反例均在案）：主审一票制（OTA 签名自指 P1 三家命中一家漏审——漏审者主审则 P1 流入实现）、DOC 一律有声表（manifest 所有权反例：会把真静默 P1 机械降 P2）、轮次自动 A-（签名自指第 4 轮才发现）、通用四态审批（代码 A 语义稀释）。正例：签名自指/8s WDT 误杀仍挡 L1；反例：loop drain/POD vs 指针/hex 大小写不挡、不开新版。独立复核 = 三份裁定文件本身。 |
